...
--Citizen.CreateThread(function()
    --while true do
        ...
        --if(IsPedInAnyVehicle(GetPlayerPed(-1), false)) and (not ownedLicenses['dmv'] or not ownedLicenses['drive']) and CurrentTest ~= 'drive' then
			--DrawMissionText("~r~You do not have a Driver's License.", 200)
        --end
...
function StopTheoryTest(success)
	...
	if success then
		...
		TriggerServerEvent('esx_dmvschool:addItem', 'identification')
	else
...
function StopDriveTest(success)
	if success then
		...
		if CurrentTestType == 'drive' then
			TriggerServerEvent('esx_dmvschool:addItemB', 'drivers_license')
		end
